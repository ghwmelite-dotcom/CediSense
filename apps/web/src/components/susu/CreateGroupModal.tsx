...
export function CreateGroupModal({ open, onClose, onSave }: CreateGroupModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60]" role="dialog" aria-modal="true" aria-labelledby="create-group-title">
      ...
      <div className="absolute inset-0 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-md sm:max-h-[90vh] sm:rounded-2xl bg-ghana-dark sm:border sm:border-white/10 sm:shadow-2xl sm:shadow-black/40 flex flex-col">
        {/* Fixed header */}
        <div className="shrink-0 px-6 pt-6 pb-3 flex items-center justify-between">
          ...
          <button type="button" onClick={handleClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-muted hover:text-white hover:bg-white/10 transition-colors" aria-label="Close">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto overscroll-contain px-6 space-y-4 pb-4" style={{ WebkitOverflowScrolling: 'touch' }}>
            {/* Group name */}
          ...
          <div className="shrink-0 px-6 pt-3 pb-6 border-t border-white/10 bg-ghana-dark sm:rounded-b-2xl" style={{ paddingBottom: 'max(1.5rem, env(safe-area-inset-bottom))' }}>
